		...
		<div class="content" id="content">
			...
			<ul class="pagination" >
				<c:if test="${pageMaker.prev}">
		        	<li><a href="itList${pageMaker.makeSearch(pageMaker.startPage - 1) }">&laquo;</a></li>
		        </c:if>
				<c:forEach begin="${pageMaker.startPage }" end="${pageMaker.endPage }" var="idx">
		        	<li <c:out value="${pageMaker.cri.page == idx?'class =active':''}"/>>
		            	<a href="itList${pageMaker.makeSearch(idx)}">${idx}</a>
		            </li>
		        </c:forEach>
				<c:if test="${pageMaker.next && pageMaker.endPage > 0}">
		        	<li><a href="itList${pageMaker.makeSearch(pageMaker.endPage +1) }">&raquo;</a></li>
			    </c:if>
			</ul>
	...
	<layout:put block="scripts" type="REPLACE">
		<script type="text/javascript">
			...
			$('#searchBtn').on("click", function(event) {
			    var url = "itList" + '${pageMaker.makeQuery(1)}'+ "&searchType="+ $("select option:selected").val() +
